...
# Prompt for generating alibis for suspects in a mystery game
ALIBI_GENERATION_PROMPT = """You are an alibi generator for a mystery game set in a biolab during a biohazard breach.
...
class AIClient:
    ...
    def generate_alibi(self):
        # Ask the AI to generate a unique alibi for a suspect
        # Returns just the alibi text as a string
        alibi_text = self.send_messages(
            [
                {
                    "role": "system",
                    "content": "You are a creative alibi generator for a mystery game. Keep responses concise and realistic.",
                },
                {
                    "role": "user",
                    "content": ALIBI_GENERATION_PROMPT,
                },
            ]
        )
        return alibi_text.strip()
